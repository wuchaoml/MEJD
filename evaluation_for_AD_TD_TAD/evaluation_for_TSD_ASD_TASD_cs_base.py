...
def evaluate_TSD_contain_NULL(path, best_file, tag_schema):
	best_P, best_R, best_F, best_epoch = 0.0, 0.0, 0.0, 0
	with open(os.path.join(path, TXT_file(best_file)), 'r', encoding='utf-8') as f_pre:
		Gold_Num = 0
		True_Num = 0
		Pre_Num = 0
		tag_schema == 'TO'
		...
		for line in pre_lines:
			...
			if pre_line[0] in ['1', '2', '3']:  # yes on gold
				gold_entity = set()
				...
				if pre_line[0] == '1':        # this line for positive
				...
				elif pre_line[0] == '2':  # this line for negative
					if len(gold_entity) == 0:   # NULL
	...
	print("\tP: ", best_P, "   R: ", best_R, "  F1: ", best_F)
	print('----------------------------------------------------\n\n')

def evaluate_TSD_ignore_NULL(path, best_file, tag_schema):
	best_P, best_R, best_F, best_epoch = 0.0, 0.0, 0.0, 0
	with open(os.path.join(path, TXT_file(best_file)), 'r', encoding='utf-8') as f_pre:
		Gold_Num = 0
		True_Num = 0
...
def evaluate_ASD(path, best_file):
	best_P, best_R, best_F, best_epoch = 0.0, 0.0, 0.0, 0
	with open(os.path.join(path, TXT_file(best_file)), 'r', encoding='utf-8') as f_pre:
		Gold_Num = 0
		True_Num = 0
		...
		for line in pre_lines:
			...
			if pre_line[0] == '1':  # yes on gold
				...
				if pre_line[1] == '1': # yes on pre
					True_Num += 1
			if pre_line[0] == '2':  # yes on gold
				Gold_Num += 1
				if pre_line[1] == '2': # yes on pre
					True_Num += 1
			if pre_line[0] == '3':  # yes on gold
				Gold_Num += 1
				if pre_line[1] == '3': # yes on pre
					True_Num += 1
			...
			for line in pre_lines:
				...
				if pre_line[0] in ['1', '2', '3']:	# yes on gold
					...
					if len(gold_entity) == 0:	# NULL
						Gold_Num += 1
						NULL_Gold_Num += 1
						if len(pre_entity) == 0 and pre_line[1] == pre_line[0]:
							True_Num += 1
							NULL_True_Num += 1
					else:	# not NULL, has entity in this sentence
						Gold_Num += len(gold_entity)
						for x in gold_entity:
							if x in pre_entity and pre_line[1] == pre_line[0]:
								True_Num += 1
				else:	# no on gold
					NO_and_O_Gold_Num += 1
...
if __name__ == '__main__':
	...
	# evaluate_ASD(args.output_dir, args.num_epochs)
	# evaluate_TSD_contain_NULL(args.output_dir, args.num_epochs, args.tag_schema)
	# evaluate_TSD_ignore_NULL(args.output_dir, args.num_epochs, args.tag_schema)
