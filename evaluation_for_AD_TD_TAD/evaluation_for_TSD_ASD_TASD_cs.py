...
def evaluate_TSD_contain_NULL(path, num_epochs, tag_schema):
	...
	print("\tP: ", best_P, "   R: ", best_R, "  F1: ", best_F, "  Epoch:  ", best_epoch)
	print('----------------------------------------------------\n\n')

def evaluate_TSD_ignore_NULL(path, num_epochs, tag_schema):
	best_P, best_R, best_F, best_epoch = 0.0, 0.0, 0.0, 0
	for index in range(num_epochs):
		file_pre = 'test_ep_' + str(index+1)
		with open(os.path.join(path, TXT_file(file_pre)), 'r', encoding='utf-8') as f_pre:
...
def evaluate_ASD(path, num_epochs):
	best_P, best_R, best_F, best_epoch = 0.0, 0.0, 0.0, 0
	for index in range(num_epochs):
		file_pre = 'test_ep_' + str(index+1)
		with open(os.path.join(path, TXT_file(file_pre)), 'r', encoding='utf-8') as f_pre:
